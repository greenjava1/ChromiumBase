...
STDMETHODIMP EnumVariant::Next(ULONG requested_count,
  ...
  ULONG available_count = ULONG{ static_cast<ULONG>(items_.size()) } - current_index_;
...
STDMETHODIMP EnumVariant::Skip(ULONG skip_count) {
  ...
  if (current_index_ + count > ULONG{ static_cast<ULONG>(items_.size())})
    count = ULONG{ static_cast<ULONG>(items_.size())} - current_index_;
...
STDMETHODIMP EnumVariant::Clone(IEnumVARIANT** out_cloned_object) {
  ...
  EnumVariant* other = new EnumVariant(ULONG{ static_cast<ULONG>(count)});
